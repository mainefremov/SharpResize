...
namespace SharpResize
{
	public class SharpResizeEngine
	{
		...
		public double[,] Resize(double[,] input, int targetSizeX, int targetSizeY)
		{
			...
			_analyDegree = inputWidth > 3000 || inputHeight > 3000 ? 1 : 3;
			var totalDegree = _analyDegree + 4;
			_corrDegree = _analyDegree + 4;
			var halfSupport = (totalDegree + 1.0D) / 2.0D;

			var zoomX = (double)targetSizeX / inputWidth;
			var zoomY = (double)targetSizeY / inputHeight;
			...
			var lengthTotalHeight = inputHeight + (int)Math.Ceiling(addBorderHeight / zoomY);
			...
			for (var l = 0; l < finalTotalHeight; l++)
			{
				...
				_indexMinHeight[l] = (int)Math.Ceiling(affineIndex - halfSupport);
				_indexMaxHeight[l] = (int)Math.Floor(affineIndex + halfSupport);
			...
			var lengthTotalWidth = inputWidth + (int)Math.Ceiling(addBorderWidth / zoomX);
			...
			for (var l = 0; l < finalTotalWidth; l++)
			{
				...
				_indexMinWidth[l] = (int)Math.Ceiling(affineIndex - halfSupport);
				_indexMaxWidth[l] = (int)Math.Floor(affineIndex + halfSupport);
		...
		private static double Beta(double x, int degree)
		{
			...
			{
				var a = x * x;
				betan = a * (a * (a * (x * 0.0069444444444444441D - 0.027777777777777776D) + 0.1111111111111111D) - 0.33333333333333331D) +
				        0.47936507936507938D;
				return betan;
			}
			...
			if (x < 2D)
			{
				betan = x *
				        (x *
				         (x *
				          (x * (x * (x * (0.050000000000000003D - x * 0.0041666666666666666D) - 0.23333333333333334D) + 0.5D) -
				           0.3888888888888889D) - 0.10000000000000001D) - 0.077777777777777779D) + 0.49047619047619045D;
				return betan;
			}
			...
			if (x < 3D)
			{
				betan = x *
				        (x *
				         (x *
				          (x * (x * (x * (x * 0.0013888888888888889D - 0.027777777777777776D) + 0.23333333333333334D) - 1.0555555555555556D) +
				           2.7222222222222223D) - 3.8333333333333335D) + 2.411111111111111D) - 0.22063492063492063D;
				return betan;
			}
